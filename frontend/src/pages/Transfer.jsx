...
const Transfer = () => {
  ...
  const validateForm = () => {
    ...
    if (!transferForm.amount || isNaN(transferForm.amount) || parseFloat(transferForm.amount) <= 0) {
      newErrors.amount = 'Please enter a valid amount greater than 0';
    } else {
      // Check if source account has sufficient funds
      const sourceAccount = accounts.find(acc => acc.id === parseInt(transferForm.fromAccount));
      if (sourceAccount && parseFloat(transferForm.amount) > sourceAccount.balance) {
  ...
  const fromAccount = transferResult ? transferResult.fromAccount : getAccountDetails(transferForm.fromAccount);
  const toAccount = transferResult ? transferResult.toAccount : getAccountDetails(transferForm.toAccount);

  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h2 className="text-2xl font-semibold text-gray-800 mb-6">Transfer Money</h2>
      ...
      {showConfirmation ? (
        ...
        >
          <div className="mb-6">
            ...
            <p className="text-gray-600">Your transfer has been scheduled successfully.</p>
          </div>
          ...
          <div className="bg-gray-50 rounded-lg p-4 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-medium">{fromAccount.type} ({fromAccount.number})</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">To</p>
                <p className="font-medium">{toAccount.type} ({toAccount.number})</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Amount</p>
                <p className="font-medium text-green-600">
                  ${transferResult ? transferResult.amount.toFixed(2) : parseFloat(transferForm.amount).toFixed(2)}
                </p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Date</p>
                <p className="font-medium">{transferResult ? transferResult.date : transferForm.transferDate}</p>
              </div>
              {(transferResult?.memo || transferForm.memo) && (
                <div className="col-span-2">
                  <p className="text-sm text-gray-500">Memo</p>
                  <p className="font-medium">{transferResult ? transferResult.memo : transferForm.memo}</p>
                </div>
              )}
            </div>
          </div>
        ...
        >
          <form onSubmit={handleSubmit}>
            {/* From Account */}
            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-700 mb-2" htmlFor="fromAccount">
                From Account
              </label>
              <select
                id="fromAccount"
                ...
                className={`w-full border ${errors.fromAccount ? 'border-red-500' : 'border-gray-300'} rounded-md p-2`}
            ...
            {/* To Account */}
            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-700 mb-2" htmlFor="toAccount">
                To Account
              </label>
              <select
                id="toAccount"
                ...
                className={`w-full border ${errors.toAccount ? 'border-red-500' : 'border-gray-300'} rounded-md p-2`}
            ...
            <div className="mb-6">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`pl-7 w-full border ${errors.amount ? 'border-red-500' : 'border-gray-300'} rounded-md p-2`}
                  placeholder="0.00"
            ...
            {/* Transfer Date */}
            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-700 mb-2" htmlFor="transferDate">
                Transfer Date
              </label>
              <input
                type="date"
                ...
                className={`w-full border ${errors.transferDate ? 'border-red-500' : 'border-gray-300'} rounded-md p-2`}
            ...
            {/* Memo */}
            <div className="mb-8">
              <label className="block text-sm font-medium text-gray-700 mb-2" htmlFor="memo">
                Memo (Optional)
              </label>
              <input
                type="text"
      ...
      >
        <h3 className="text-md font-semibold text-blue-800 mb-2">Quick Tips</h3>
        <ul className="text-sm text-blue-700 list-disc list-inside space-y-1">
          <li>Internal transfers between your accounts are processed immediately</li>
          <li>External transfers may take 1-3 business days to complete</li>
          <li>You can set up recurring transfers from the Scheduled Transfers section</li>
